...
# Define the DAG
with DAG(
    dag_id="Flight_Booking_Pipeline_dag",
    ...
    gcs_bucket = Variable.get("gcs_bucket", default_var="flights-booking-pipeline")
    bq_project = Variable.get("bq_project", default_var="project-46b39fb7-a676-44c5-970")
    bq_dataset = Variable.get("bq_dataset", default_var=f"flight_booking_pipeline_data_{env}")
    ...
    file_sensor = GCSObjectExistenceSensor(
        ...
        bucket=gcs_bucket,
        object=f"flights-booking-analysis/source-{env}/flight_booking.csv",  # Full file path in GCS
        google_cloud_conn_id="google_cloud_default",  # GCP connection
        timeout=300,  # Timeout in seconds
        poke_interval=30,  # Time between checks --retry 
    ...
    batch_details = {
        "pyspark_batch": {
            "main_python_file_uri": f"gs://{gcs_bucket}/flights-booking-analysis/spark-job/spark_transformation_job.py",  # Main Python file
        ...
        "environment_config": {
            "execution_config": {
                "service_account": "972282312012-compute@developer.gserviceaccount.com",
